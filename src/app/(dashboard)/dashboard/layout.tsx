...
export default function DashboardLayout({
  ...
  const leftDesktopOpen = useAppSelector(
    (state) => state.sidebar.leftDesktopOpen
  );
  const leftMobileOpen = useAppSelector(
    (state) => state.sidebar.leftMobileOpen
  );
  const leftSection = useAppSelector((state) => state.sidebar.leftSection);
  const selectedSection = useAppSelector(
    (state) => state.sidebar.selectedSection
  );
  const { isAuthenticated, name, role, loading } = useAppSelector(
    (state) => state.user
  );
...
//         const response = await uploadRawLoader(
//           file,
...
//         alert(
//           `❌ Raw Loader Upload failed: ${error.message || "Unknown error"}`
//         );
//       } finally {
          ...
          <ScrollArea className="flex-1">
            <div className="p-3 lg:p-4 space-y-1 lg:space-y-2">
              {sidebarItems.map((item) => (
                <div
                  key={item.section}
                  className={`flex items-center gap-3 p-2 lg:p-3 rounded-lg cursor-pointer transition-colors ${
                    leftDesktopOpen ? "justify-start" : "justify-center"
                  } ${
